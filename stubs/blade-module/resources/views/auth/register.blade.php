<x-{{moduleName}}::guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div>
            ...
            <x-{{moduleName}}::text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
            <x-{{moduleName}}::input-error :messages="$errors->get('name')" class="mt-2" />
        </div>

        <!-- Email Address -->
        <div class="mt-4">
            <x-{{moduleName}}::input-label for="email" :value="__('Email')" />
            <x-{{moduleName}}::text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email')" required autocomplete="username" />
        ...
        <div class="flex items-center justify-end mt-4">
            ...
            <x-{{moduleName}}::primary-button class="ms-4">
                {{ __('Register') }}
